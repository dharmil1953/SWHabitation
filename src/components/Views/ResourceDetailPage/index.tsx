...
import ResourceDetailHeroSection from "./ResourceDetailHeroSection";
import locales from "../../../../lib/locales";
import clsx from "clsx";
import SideContentTitle from "./SideContentTitle";
import RecommendedResourceSection from "./RecommendedResourceSection";
import ResourceDetailShareButtons from "./ResourceDetailShareButtons";
import {
  BackgroundBorder,
  BackgroundColor,
  BackgroundFill,
...
import PreviewModalSection from "../../Commons/PreviewModalSection";
export interface ResourceDetailPageProps {
  block?: ResourceDetailPageType;
}
const ResourceDetailPage: React.FC<ResourceDetailPageProps> = ({ block }) => {
  ...
  return (
    <div>
      {block && <ResourceDetailHeroSection block={block} />}
      <div>
        <div className="container">
          <div className="s(1100):pt-[48px] pt-[90px] flex flex-col-reverse lg:flex-row justify-between s(640):mb-[100px] s(1100):mb-[150px] mb-[180px]">
            <div className="w-[100%] max-w-[20%] mr-[0] lg:mr-[2em] sticky top-[2em] z-10 h-full  lg:block   mt-[2em] lg:mt-[0] self-start space-y-8   ">
              <Link
                ariaLabel={locales.en.BACK_TO_RESOURCES}
                className={clsx(
                  "capitalize text-20 font-normal flex items-center gap-2.5 justify-center py-4 group border hover:text-white bg-transparent transition-all duration-300 ease-linear",
                  BackgroundBorder[backgroundColor],
                  BackgroundHover[backgroundColor],
                  ResourcesText[backgroundColor],
                  BackgroundHover[backgroundColor]
                )}
              ...
              >
                <LeftArrow
                  className={clsx(
                    "w-4 h-4 group-hover:[&>path]:fill-white ",
                    BackgroundBorder[backgroundColor],
                    BackgroundFill[backgroundColor]
                  )}
                />
                {locales.en.BACK_TO_RESOURCES}
              </Link>
              <div>
                <SideContentTitle
                  backgroundColor={backgroundColor}
                  sideTitle={sideContentTitles}
                />
              </div>
            </div>
            <div className="blogInner s(1024):w-[100%] s(1100):w-full w-[72%] s(1024):pt-[17em] pt-[0]">
              {getpublishDate && (
                <span className="text-base text-theme-black pb-5 block">
                  {`${getpublishDate}`}
                </span>
              )}
              ...
              {content?.custom_rich_text && (
                <div
                  className="[&>p]:text-theme-black [&>p]:text-20 [&>p]:font-normal [&>p]:pb-6 [&>p]:leading-[1.5] [&>h6]:text-theme-black [&>h6]:text-24  [&>h6]:font-semibold  [&>h6]:mb-4
								 "
                >
                  {content?.custom_rich_text && (
                    <RichText block={content} isClickable={true} />
                  )}
                </div>
              )}
              {author?.name && (
        ...
        {recommendedResources?.length > 0 && (
          <RecommendedResourceSection block={recommendedResources} />
        )}
      </div>
    </div>
  );
