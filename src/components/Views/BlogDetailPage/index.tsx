...
import RecommendedBlogSection from "./RecommendedBlogSection";
import clsx from "clsx";
import BlogDetailShareButtons from "./BlogDetailShareButtons";
...
const BlogDetailPage: React.FC<BlogDetailPage> = ({ block }) => {
  ...
  return (
    <div>
      {block && <BlogDetailHeroSection block={block} />}
      <div className="mb-[6rem]">
        <div className="container">
          <div className="flex flex-col-reverse lg:flex-row justify-between s(640):mb-[100px] s(1100):mb-[150px] mb-[180px] gap-10">
            <div className="w-full max-w-[20%]  sticky top-[2em] z-10 hidden lg:block px-[1em] lg:px-[0] mt-[2em] lg:mt-[0] self-start space-y-8 ">
              <Link
                ariaLabel={locales.en.BACK_TO_BLOGS}
                className="capitalize text-20 font-normal flex items-center gap-2.5 justify-center py-4 group text-theme-black border border-theme-black   bg-transparent  px-5 hover:bg-theme-black hover:text-white transition duration-200 group"
                to={routes.blog_index()}
              >
                <LeftArrow className="group-hover:[&>path]:fill-white" />
                {locales.en.BACK_TO_BLOGS}
              </Link>
              <SideContentTitle sideTitle={sideContentTitles} />
            </div>
            <div
              className="blogInner s(1024):w-[100%] s(1100):w-full w-[72%] s(1024):pt-[17em] pt-[0]"
              id="blogContent"
            >
              {getpublishDate && (
              ...
              {content?.custom_rich_text && (
                <div className="[&>p]:text-theme-black s(1280):[&>p]:text-18 [&>p]:text-20 [&>p]:font-normal [&>p]:pb-6 [&>p]:leading-[1.5] [&>p>br]:hidden  [&>h4]:text-26 [&>h4]:mb-6 [&>h4]:mt-4 ">
                  {content?.custom_rich_text && (
              ...
              {author?.name && (
                <div className="s(640):flex-col flex flex-row justify-between items-center mt-10 pt-5 border-t-[1px] border-theme-gray">
                  <div className="s(640):mb-6 s(480):text-center text-left">
                    <h5
                      className={clsx(
                  ...
                  <div className="flex  s(400):flex-col items-center gap-[12px]">
                    <div
